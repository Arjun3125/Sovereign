...
class MinisterSynthesizer:
    ...
    def synthesize(
        ...
        # After generating structured result, emit per-citation RAG traces (best-effort)
        try:
            from core.rag.trace import log_rag_trace
            minister_interp = result.get("rationale") or result.get("advice") or ""
            citations = result.get("citations", [])
            # Normalize retrieved chunks for lookup
            all_chunks = {}
            for cat in ["support", "counter", "neutral"]:
                for c in (retrieved.get(cat, []) or []):
                    all_chunks[c.get("chunk_id")] = c

            for cit in citations:
                chunk_id = cit.get("chunk_id") or cit.get("id")
                if not chunk_id:
                    continue
                chunk = all_chunks.get(chunk_id)
                book = cit.get("book_id") or (chunk.get("book_id") if chunk else "")
                chapter = cit.get("chapter_title") or (chunk.get("chapter_title") if chunk else "")
                principle = cit.get("principle_id") or (chunk.get("principle_id") if chunk else chunk_id)
                principle_summary = (chunk.get("text")[:400]) if chunk and chunk.get("text") else ""
                score = float(chunk.get("score", 0.0)) if chunk else 0.0
                # Log trace
                try:
                    log_rag_trace(decision_id, book, chapter, principle, minister_interp, score)
                except Exception:
                    pass
        except Exception:
            pass
