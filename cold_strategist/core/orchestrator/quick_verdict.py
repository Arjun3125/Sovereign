...
class QuickEngine:
    ...
    def run(
        ...
        # Telemetry: append DecisionEvent and MinisterEvent entries (best-effort)
        try:
            from core.telemetry.store import TelemetryStore
            from core.telemetry.models import DecisionEvent, MinisterEvent
            ...
            t.append(
                ...
                DecisionEvent(
                    ...
                    context_summary=(getattr(context, 'summary', '') or getattr(context, 'raw_text', '')),
                    stakes=getattr(state, 'stakes', 'medium'),
                    emotional_load=emotional_load,
                    urgency=getattr(state, 'urgency', 0.0),
            ...
            # Minister events
            for ml in verdict.minister_lines:
                mid = uid()
                t.append(
                    "minister_events",
                    MinisterEvent(
