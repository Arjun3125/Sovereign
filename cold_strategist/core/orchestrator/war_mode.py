...
class WarLogEntry:
    ...
    # Audit flags
    all_minister_outputs_logged: bool = False
    rag_traces_logged: bool = False
...
class WarModeEngine:
    ...
    def __init__(self):
        ...
        # Controls for which soft filters are relaxed in War Mode
        self.relaxations = {
            "moral_filter": False,
            "reputation_safeguard": False,
            "counter_pattern_suppression": True,
        }
    ...
    def apply_filters(self, minister_output):
        """
        Apply War Mode relaxations to a minister's output.

        This is intentionally minimal: it demonstrates how the posture
        shifts under War Mode. Callers must still ensure no illegal
        or individually-targeted advice is produced.
        """
        try:
            if self.relaxations.get("counter_pattern_suppression"):
                # prefer direct framing: remove counter-moralization step if available
                if hasattr(minister_output, "remove_counter_moralization"):
                    return minister_output.remove_counter_moralization()
        except Exception:
            pass

        return minister_output
    ...
    def log(self, assessment: WarAssessment, notes: Optional[str] = None):
        ...
        entry = WarLogEntry(
            ...
            # by default assume audit flags will be set by the caller
            all_minister_outputs_logged=True,
            rag_traces_logged=True,
            override_required=bool(notes),
        )
        self.logs.append(entry)
        # Also write a telemetry DecisionEvent for audit (best-effort)
        try:
            from core.telemetry.store import TelemetryStore
            from core.telemetry.models import DecisionEvent
            ...
            t.append(
                ...
                DecisionEvent(
                    id=uid(),
                    session_id=None,
                    context_summary=assessment.recommended_posture,
                    stakes="high",
                    emotional_load=0.0,
                    urgency=0.0,
                    fatigue=0.0,
